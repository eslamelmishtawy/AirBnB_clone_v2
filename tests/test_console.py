...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_fs_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User name="Eslam" age=28 height=175')
            mdl_id = cout.getvalue().strip()
            self.assertIn('User.{}'.format(mdl_id), storage.all().keys())
            clear_stream(cout)
            cons.onecmd('show User {}'.format(mdl_id))
            self.assertIn("'name': 'Eslam'", cout.getvalue().strip())
            self.assertIn("'age': 28", cout.getvalue().strip())
            self.assertIn("'height': 175", cout.getvalue().strip())
    ...
    def test_db_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User email="test@test.com" password="123456"')
            mdl_id = cout.getvalue().strip()
            dbc = MySQLdb.connect(
    ...
    def test_db_show(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            obj = User(email="test@test.com", password="123456")
            dbc = MySQLdb.connect(
                host=os.getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=os.getenv('HBNB_MYSQL_USER'),
            ...
            self.assertIn('test@test.com', result)
            self.assertIn('123456', result)
            self.assertIn('test@test.com', cout.getvalue())
            self.assertIn('123456', cout.getvalue())
            cursor.close()
            dbc.close()
    ...
    def test_db_count(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            cons.onecmd('create State name="Texas"')
            clear_stream(cout)
